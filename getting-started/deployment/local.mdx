...
1. Open a new Terminal window and run the following command to create a folder and download the OpenOps release files:
    ...
    unzip openops-dc-0.2.5.zip && cp -n .env.defaults .env
   ```
...
```powershell
Invoke-WebRequest -Uri "https://github.com/openops-cloud/openops/releases/download/0.2.5/openops-dc-0.2.5.zip" -OutFile "openops-dc-0.2.5.zip"
Expand-Archive -Path "openops-dc-0.2.5.zip" -DestinationPath .
