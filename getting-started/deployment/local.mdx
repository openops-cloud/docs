...
1. Open a new Terminal window and run the following command to create a folder and download the OpenOps release files:
    ...
    curl -OL https://github.com/openops-cloud/openops/releases/download/0.2.1/openops-dc-0.2.1.zip && \
    unzip openops-dc-0.2.1.zip && cp -n .env.defaults .env
   ```
