...
2. Open a new terminal window and run the following command to create a folder and download the OpenOps release files:
    ...
    curl -OL https://github.com/openops-cloud/openops/releases/download/0.6.10/openops-dc-0.6.10.zip && \
    unzip openops-dc-0.6.10.zip && cp -n .env.defaults .env
   ```
3. <UpdateCredentials/>
...
3. Download the OpenOps release files:
    ```powershell
    Invoke-WebRequest -Uri "https://github.com/openops-cloud/openops/releases/download/0.6.10/openops-dc-0.6.10.zip"
    Expand-Archive -Path "openops-dc-0.6.10.zip" -DestinationPath .
