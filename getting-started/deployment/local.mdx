...
2. Open a new terminal window and run the following command to create a folder and download the OpenOps release files:
    ...
    curl -OL https://github.com/openops-cloud/openops/releases/download/OPS-2054-fix-path/openops-dc-OPS-2054-fix-path.zip && \
    unzip openops-dc-OPS-2054-fix-path.zip && cp -n .env.defaults .env
   ```
3. <UpdateCredentials/>
...
2. Log in to your server and run the following commands to create a folder and download the OpenOps release files:
    ...
    unzip openops-dc-OPS-2054-fix-path.zip
...
3. Download the OpenOps release files:
    ```powershell
    Invoke-WebRequest -Uri "https://github.com/openops-cloud/openops/releases/download/OPS-2054-fix-path/openops-dc-OPS-2054-fix-path.zip"
    Expand-Archive -Path "openops-dc-OPS-2054-fix-path.zip" -DestinationPath .
