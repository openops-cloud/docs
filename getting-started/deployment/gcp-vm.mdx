...
5. Under **Boot disk**:
    ...
    - Click **Select**.
6. Under **Firewall**, check **Allow HTTP traffic**. This will automatically create a firewall rule to open port 80.
...
1. First, stop all running containers:
    ...
    sudo docker compose down
    ```
2. Download and unzip the newer release into the same location where you initially installed it:
    ```shell
    wget https://github.com/openops-cloud/openops/releases/download/0.2.2/openops-dc-0.2.2.zip
    python3 -m zipfile -e openops-dc-0.2.2.zip .
    ```
    > Replace the version in the URL with the actual version you want to install.
