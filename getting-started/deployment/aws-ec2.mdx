...
For production usage, it’s recommended to enable TLS (HTTPS). You can achieve this by adding an EC2 Load Balancer to serve traffic to your EC2 instance, as shown in the next section on custom domains.

In addition to the security aspect, enabling TLS also ensures that [workflow templates](/workflow-management/workflow-templates/) load properly in all browsers.

## Accessing OpenOps via a custom domain over TLS (HTTPS)

If you want to access your OpenOps instance via a custom domain (e.g., `https://openops.your-company.com`) instead of its public IP address, you need to set up an AWS Application Load Balancer (ALB) with a valid TLS certificate from AWS Certificate Manager (ACM), then create a new DNS record and update your OpenOps configuration.
