...
2. In CloudShell, create a new directory and download the release files:
    ...
    wget https://github.com/openops-cloud/openops/releases/download/0.4.1-aws-cli-fix/openops-dc-0.4.1-aws-cli-fix.zip && \
    unzip -o openops-dc-0.4.1-aws-cli-fix.zip && cp --update=none .env.defaults .env
    ```
