...
1. In CloudShell, create a new directory and download the release files:
    ...
    python3 -m zipfile -e openops-docker-compose-0.1.5.zip . && cp --update=none .env.defaults .env
    ```
2. Update the application URL to use the instance’s public IP address:
    ```shell
    sed -i 's/http:\/\/localhost/http:\/\/'$(wget -qO - http://ipv4.myip.wtf/text)'/g' .env
    ```
...
Version 0.1.5 introduces a new gateway container, *nginx*, to handle incoming traffic. This change requires updating several environment variables. You can either override the `.env` file entirely (`cp .env.default .env`) or update specific keys:
```shell
