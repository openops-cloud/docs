...
3. In the **Summary** section on the right, click **Launch instance**:
    ![Launch instance](/images/deploy-aws-launch-instance-2.png)
...
5. Configure the network settings and open the necessary ports:
    ...
    3. Add two new rules by clicking **Add rule** each time:
        ...
        2. In the second rule, for **Type**, choose **SSH**. Click the text box next to **Source**, and search for `ec2-instance-connect`. Select the prefix list with the full name similar to `com.amazonaws.us-east-1.ec2-instance-connect`, depending on your region. This allows you to connect to the instance via AWS CloudShell.
...
2. In CloudShell, create a new directory and download the release files:
    ...
    wget https://github.com/openops-cloud/openops/releases/download/0.6.6/openops-dc-0.6.6.zip && \
    unzip -o openops-dc-0.6.6.zip && cp --update=none .env.defaults .env
    ```
