...
2. In CloudShell, create a new directory and download the release files:
    ...
    wget https://github.com/openops-cloud/openops/releases/download/0.4.3-nginx-config-fix/openops-dc-0.4.3-nginx-config-fix.zip && \
    unzip -o openops-dc-0.4.3-nginx-config-fix.zip && cp --update=none .env.defaults .env
    ```
