...
1. Create a new directory and download the OpenOps release files:
    ...
    wget https://github.com/openops-cloud/openops/releases/download/0.2.0/docker-compose-release-0.2.0.zip && \
    python3 -m zipfile -e openops-docker-compose-0.2.0.zip . && cp -n .env.defaults .env
    ```
...
2. To update your OpenOps Docker Compose installation, unzip the new release into the same location where you initially installed it:
    ```shell
    cd openops && wget https://github.com/openops-cloud/openops/releases/download/0.2.0/docker-compose-release-0.2.0.zip && \
    python3 -m zipfile -e openops-docker-compose-0.2.0.zip .
    ```
