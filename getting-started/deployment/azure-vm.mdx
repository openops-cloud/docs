...
2. Create a new directory and download the OpenOps release files:
    ...
    wget https://github.com/openops-cloud/openops/releases/download/0.3.2-download-url-OPS-2054/openops-dc-0.3.2-download-url-OPS-2054.zip && \
    unzip -o openops-dc-0.3.2.zip && cp -n .env.defaults .env
    ```
...
You can now access the application by navigating to the public IP address of your virtual machine, for example, `http://20.20.20.20`. If you did not update credentials in the `.env` file, the default username is `admin@openops.com` and the password is `please-change-this-password-1`.
