...
1. Create a new directory and download the OpenOps release files:
    ...
    wget https://openops.s3.amazonaws.com/releases/0.1.5/openops-docker-compose-0.1.5.zip && \
    python3 -m zipfile -e openops-docker-compose-0.1.5.zip . && cp -n .env.defaults .env
    ```
...
2. To update your OpenOps Docker Compose installation, unzip the new release into the same location where you initially installed it:
    ```shell
    cd openops && wget https://openops.s3.amazonaws.com/releases/0.1.5/openops-docker-compose-0.1.5.zip && \
    python3 -m zipfile -e openops-docker-compose-0.1.5.zip .
    ```
...
### Updating from 0.1.4 to 0.1.5

Version 0.1.5 introduced a new gateway container, *nginx*, to handle incoming traffic. This change requires updating several environment variables. You can either override the `.env` file entirely (`cp .env.default .env`) or update specific keys:
```shell
sed -i 's/http:\/\/tables/http:\/\/openops-tables/' .env
sed -i 's/http:\/\/analytics/http:\/\/openops-analytics/' .env
sed -i 's/:3001//' .env
sed -i 's/:3002//' .env
```
