...
6. Log in to ECR:
    ...
    ```
7. Inside the `openops-installation` directory, open a terminal and perform the following command to download Docker images:
    ```shell
    docker compose pull
    ```
    Wait until all images are pulled:
    ![All images are pulled](/images/qsg-pulled.png)
8. To launch OpenOps, run this in your terminal:
    ```shell
    docker compose up -d
    ```
...
9. If you want to configure Slack approvals, please see [setting up Slack on local environment](/getting-started/deployment/local#configuring-slack-approvals)
