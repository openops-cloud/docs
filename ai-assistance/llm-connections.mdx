...
6. In the **Model** dropdown, select one of the models your LLM provider supports. (If you're configuring Azure OpenAI, select **Custom** instead of a model and complete other [Azure OpenAI-specific steps](#azure-openai).)
...
10. (Optional) Use the **Provider settings** and **Model settings** fields to specify custom parameters as JSON. The JSON schema varies depending on the chosen provider and model:
    * See [Azure OpenAI](#azure-openai) and [Google Vertex AI](#google-vertex-ai) instructions for custom provider settings that these providers require.
    * If you've selected OpenAI, you can use **Provider settings** for JSON that you'd normally pass to the `createOpenAI` function, and **Model settings** for JSON that you'd normally pass to the `streamText` function. For more details, see the [OpenAI documentation](https://platform.openai.com/docs/api-reference).
...
Some LLM providers require additional configuration beyond the API key and/or specifying other non-standard settings. This section offers guidance for some of the most common providers in this category.
...
When configuring an AI connection where Google Vertex AI serves as the provider, you need to add the following to the **Provider settings** field:
