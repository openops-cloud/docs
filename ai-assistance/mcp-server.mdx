---
title: 'MCP Server'
description: 'How to connect AI agents such as Claude Code and Codex to OpenOps'
icon: 'plug'
---
...
<AccordionGroup>
  <Accordion title="Claude Code" defaultOpen>
    ...
    ![The /mcp server list in Claude Code](/images/claude-code-mcp-list.png)
    ...
    ![The openops server details in Claude Code with the Authenticate option](/images/claude-code-mcp-authenticate.png)
...
This section is for OpenOps administrators. The MCP server is disabled by default and requires OpenOps 0.7.0 or later. Agents sign in through OAuth, which OpenOps serves only over HTTPS; plain `http` is accepted only when `OPS_PUBLIC_URL` is `http://localhost`.
...
The [OpenOps Helm chart](https://github.com/openops-cloud/helm-chart) version 0.7.0 or later includes the MCP server as an opt-in component. Add the following to your values file, then upgrade the release:
...
The chart derives the remaining OAuth settings from `publicUrl`. See the MCP section of the chart's README for details, including how to supply the secret from an external secret manager.
