...
Creates the `OpenOpsApp` role with permissions to run workflows from the OpenOps template catalog. You can enable or disable certain permission sets (read-only is mandatory, others are optional).
