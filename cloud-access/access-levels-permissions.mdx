...
OpenOps images and the development container preinstall common Azure CLI extensions and enable dynamic install without prompts. Extensions are stored in `/opt/azure/config/cliextensions`.
...
Notes
- You can still add other extensions as needed with `az extension add --name <ext>`
- Dynamic install is set via `az config set extension.use_dynamic_install=yes_without_prompt`
