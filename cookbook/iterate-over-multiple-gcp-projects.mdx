...
## 4. Add a Google Cloud step inside the loop and use the project ID in the command
...
In the action's properties, select the connection that enables access to multiple projects. Select any project in the **Default Project** dropdown. In the **Command** field, enter a command that you want to run. For example, if you want to list active Compute Engine instances in each project, enter the following:

```shell
gcloud compute instances list --project= --format=json
```

Set the caret after the `--project=` flag. Use **Data Selector** to expand **Loop on Items**, then **item**, then click **Insert** alongside the **projectId** property:

![Selecting an accountId](/images/cookbook/gcp-data-selector-projectid.png)

The resulting command in the properties pane will look like this, injecting the current project ID into the command template:

![A command with a dynamic project ID](/images/cookbook/gcp-command-inside-loop.png)

In a similar manner, you can inject the project ID using **Data Selector** in other Google Cloud actions that you insert inside the loop.

You can [download the workflow](https://docs.openops.com/workflows/multiple-gcp-projects.json) described above and [import](/workflow-management/import-export/#importing-workflows) it to your OpenOps instance.
