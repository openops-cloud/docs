...
Inside the loop, add an Azure action that you want to perform for each subscription — for example, another **Azure CLI** action.

In the action's properties, select the connection that enables access to multiple subscriptions.

Select any subscription in the **Subscriptions** dropdown.

In the **Command** field, enter the command you want to run. For example, to list running VMs in each subscription, enter:
...
The resulting command in the properties pane will look like this, dynamically injecting the current subscription ID into the command template:

![A command with a dynamic subscription ID](/images/cookbook/azure-command-inside-loop.png)

In the same way, you can use **Data Selector** to inject the subscription ID into other Azure actions inside the loop.
