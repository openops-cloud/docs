...
[CloudHealth](https://www.vmware.com/products/app-platform/tanzu-cloudhealth) is a cloud cost management and optimization platform that helps companies manage their multi-cloud environments consisting of AWS, Azure, Google Cloud, private and hybrid cloud setups.
...
3. A Jira Cloud instance to file issues to.

## Setting up connections to services

On several steps of the tutorial, OpenOps will need to interact with CloudHealth, Slack, and Jira. To enable this interaction, you need to configure what OpenOps calls "connections" to these services.

While you could do this as you go, it's easier to set up connections in advance and use them throughout the tutorial.

In the left pane in your OpenOps instance, click **Connections**.
...
In the **API Key** field in the dialog, you need to enter your CloudHealth API key. If you don't have it at hand, follow the instructions in the dialog to generate an API key. Once you have the key, paste it into the **API Key** field and click **Save**.
...
Click **Connect**. This will open a pop-up window that asks you to authorize OpenOps to connect to your Slack workspace:
...
The goal of this workflow is to get all project names from AWS instance metadata stored in CloudHealth, and write them to an OpenOps table. This way, the EC2 rightsizing workflow that you're going to write next can look up project owners based on project names.
...
Now that you have a list of all project names, create a loop to go over each of them and decide what to do with them.
...
At the top of the properties pane, click **Edit Step Name** and rename the step to _For each project name_.
...
If a project is already listed in the table and a project owner email is already defined for it, you don't need to update the existing record to avoid overwriting project emails that were added before.

First, let's see if the project name is already in the table.

In the workflow editor, click the **+** button _inside_ the loop:
...
At the top of the properties pane, click **Edit Step Name** and rename the step to _Get project record_.
...
Click the **Test** tab in the properties pane. Under **Step output**, click **Test Step**. After testing, the output is a JSON object that represents the search results in the OpenOps table. Since the table is empty so far, the output looks like this:
...
This is a problem because the configuration of the next step assumes a found record. In cases like this, OpenOps allows you to override the actual testing output. To do this, click **Sample output** in the **Test** tab. In the **Output** area, paste the following JSON, then click **Apply**:
...
Now, if the project name is already in the table, you need to check if a project owner email is defined for it. If it is, you don't need to update the existing record.
...
At the top of the properties pane, click **Edit Step Name** and rename the step to _Is email already defined?_.

Under **Continue If**, click the left field with a placeholder that reads _First value_. In **Data Selector**, expand **5. Get project record**, then **items**, then **items[1]**. Click **Insert** alongside the **Owner email** entry:
...
Click the **Test** tab in the properties pane. Under **Step output**, click **Test Step**. After testing, the output is a JSON object that represents the condition evaluation:
...
Now, you need to add a record with the project name to the OpenOps table, unless it's already there and the owner email is already defined.
...
In the workflow editor, click the trigger step. When a dropdown with types of triggers opens, select **Schedule**, then **Every Day**.
...
Click **Sample output** in the **Test** tab. In the **Output** area, paste the following JSON, then click **Apply**:
...
At the top of the new step's properties pane, click **Edit Step Name** and rename the step to _Set to Dismissed_.
