...
The following are details on the four actions available in the Cloudability block.
...
### Custom API Call
Allow making arbitrary calls to any [Cloudability API](https://www.ibm.com/docs/en/cloudability-commercial/cloudability-standard/saas?topic=api-getting-started-cloudability-v3) endpoint:
...
Cloudability recommendations often need to be routed to the appropriate resource owners for review and action. OpenOps workflows can help you map cloud resources to their owners using tag values in the `tagMappings` array that the *Get Recommendations* action returns. Further steps of your workflow may add owners or departments in the preconfigured [*Tag-owner mapping* table](/reporting-analytics/tables#tag-owner-mapping).

If your Cloudability instance returns specific owners, your workflow can add their contacts to the *Tag-owner mapping* table automatically. If it doesn't, you can fill the table manually once for each business unit to make sure that subsequent workflow runs know which team member should receive notifications or approval requests.
