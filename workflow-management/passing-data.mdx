...
1. If there was a conditional statement, such as **Condition** or **Split**, before your current action, then your current action will not have access to the outputs of any actions inside the branches of the conditional statement. If you need output of actions in these branches to be available after the conditional statement completes, make sure to save it elsewhere, such as into OpenOps’ internal storage with the **Put** action in the **Storage** group. You can then read from the storage using the **Get** action in the same group.
2. This is also true for loops: the output of actions that occur inside a loop are not available outside the loop. If you need any data from the loop after the loop completes, make sure to save the data to storage while still inside the loop.
