...
Take time to expand the items before inserting them to understand the type of data they contain and whether they're the right fit for your needs. For example, if you're inside a loop, you may not need the entire object in the current loop iteration — expand it until you find the specific value you need:
![Selecting a deeply nested object property](/images/cloudhealth/opportunity-id-awsInstanceId.png)

If a property provides a fixed list of values (such as a dropdown) but you're not happy with any of them, click the **Dynamic** toggle at the top right of the property. This turns the property to a regular input where you can use the **Data Selector** to select a custom value:
