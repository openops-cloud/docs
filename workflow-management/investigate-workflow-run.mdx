...
By default, OpenOps times out if a workflow runs for 10 minutes. This applies both to scheduled runs and test runs.

If you build long-running workflows, you may want to extend the default timeout. To do this, open the `.env` file in your OpenOps installation directory and add a new environment variable called `OPS_FLOW_TIMEOUT_SECONDS`. Set the value to a higher timeout in seconds: for example, `900` for 15 minutes, `3600` for 1 hour, etc.
