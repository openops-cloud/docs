...
* **Loop on Items**: iterate over a list of items obtained in a prior step, executing each iteration sequentially. All iterations run to completion regardless of failures or pauses in individual iterations. After all iterations finish, the loop’s final state reflects the outcomes of all iterations: if any iteration fails, the loop step fails; if any iteration pauses, the loop step pauses.
