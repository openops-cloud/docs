...
* **HTTP**: send HTTP requests to any API or respond to HTTP requests. You can add reusable auth headers to your HTTP connection (for example, Authorization). Per‑step headers in the action override headers defined on the connection when keys match.
