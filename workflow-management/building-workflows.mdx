...
As you build your workflow, you should test individual actions in isolation. Once you're done editing and testing actions, you should also [test the entire workflow](#testing-the-entire-workflow) to make sure that everything works as expected.
...
If you don't have a connection defined when working on an action, or if you want to avoid running the action against a live system for any other reason, open the **Sample output data** tab. Once there, paste the expected output of the API call that the action performs, then click **Apply**:
...
All actions that use sample output data have a corresponding label in the workflow graph:
...
As soon as you have implemented all steps, including a trigger and actions, it's time to test your entire workflow. If all the steps in your workflow are complete, you'll see the **Test Workflow** button at the top of the workflow editor, right above the trigger node:
