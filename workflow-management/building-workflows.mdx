...
If you need to reuse an action in multiple places, you can **copy/paste** it. This copies an action along with all its property values, saving you the trouble of reconfiguring it. For example, you can copy/paste actions between [conditional branches](/workflow-management/conditional-branching/) that often end up sharing a part of their containing actions.

You can use the regular copy/paste keyboard shortcuts (**Ctrl+C/V** or **Cmd+C/V** on Mac). When you paste this way, the copied action is inserted after the action that is currently selected in the workflow graph.

You can also use the right-click context menu of any action to copy it:
![Copying an action](/images/workflow-editor-action-context-menu-copy.png)
...
* Right-click a loop action and select **Paste inside loop**. This inserts as the first action inside the loop:
...
Note that copy/paste only works within a single workflow — you can't copy an action from one workflow to another. Another limitation is that copy/paste is only available in OpenOps installations served via HTTPS.
