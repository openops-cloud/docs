...
* You can get AI assistance in many actions that involve writing CLI commands (AWS CLI, Azure CLI, Google Cloud CLI) or SQL queries (AWS Athena, Snowflake, Databricks, BigQuery). To generate commands and queries, you'll need to [connect OpenOps to an LLM provider](/cloud-access/llm-connections) of your choice. OpenAI, Anthropic, Perplexity, and a dozen other LLM providers are supported.
...
As you build your workflow, you should test individual actions in isolation. Once you've done editing and testing actions, you should also [test the entire workflow](#testing-the-entire-workflow) to make sure that everything works as expected.

### Testing individual actions

Underneath the properties pane for every action, there's a section with two tabs: **Step output** and **Sample output data**.

In the **Step output** tab, you can click **Test Step** to test or retest the action in isolation. "Testing" in this context means running the action against a live system that your action is configured to connect to.
...
If you don't have a connection defined when you work on an action, or if you want to avoid running the action against a live system for any other reason, open the **Sample output data** tab. Once there, paste the expected output of the API call that the action performs, then click **Apply**:
![Sample output data](/images/workflow-editor-action-sample-output-data.png)

This lets you avoid running the action against a live system while still being able to provide input data for subsequent actions in your workflow.

All actions that use sample output data have the corresponding label in the workflow graph:
