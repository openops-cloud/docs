---
title: 'OpenOps Slack App'
description: 'Use the OpenOps AI Agent from Slack'
icon: 'robot'
---

<Info>
**OpenOps Slack App** is an Enterprise edition feature. See [pricing](https://www.openops.com/pricing/) for details.
</Info>

The OpenOps Slack App lets you interact with OpenOps AI from Slack. Ask about workflows, runs, connections, and FinOps in natural language without leaving your team's workspace.
...
Before you can use the app in Slack, link your Slack account to OpenOps:
...
![Using the OpenOps app in Slack](/images/paid/slack-bot-usage.png)

You can interact with the OpenOps app in two primary ways, plus several other methods for different scenarios.
...
Mention the app in any channel where it's installed to start a conversation that everyone in the channel can see. Useful when several people need the same information.
...
The app supports slash commands for environment and channel defaults. Responses are **ephemeral** (only you see them).
...
Channel defaults set a shared environment and project for a channel so the app doesn't ask every time.
