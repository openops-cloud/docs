...
The last step is to add the logic that will be executed when the action is run. This logic will make a request to the YouTrack API to update the status of the issue. In doing so, it will use the inputs provided by the user in the properties we've added above.
...
* The `run()` function receives the `context` object that contains lots of data about the current action and the workflow's execution state, including the block authentication (`context.auth`) and the values of all the action's properties (`context.propsValue`).
* The first call to the YouTrack API is made to make sure that the specified issue exists. In that call, we're using the values of both the **Project** dropdown and the **Number from issue ID** input field.
* If the issue exists, we're looking for a part of its data representing the status field. Once that is found, another YouTrack API call is made, this time to update the status to the value that was specified in the **New Status** dropdown.
* The updated issue data is returned from the `run()` function, and this is what the user will see as the output of the executed action.

If we go to the workflow editor now and test the action, here's the kind of output we should see:

![The output of the Change Issue Status action](/images/contributing/youtrack-change-status-output.png)

In this case, we're simply returning the body of the response from the YouTrack API, but it's entirely possible to return an object with a different shape, this is entirely up to you.

This wraps up the "Change issue status" action. Since it used a few dynamic properties, and the YouTrack API isn't always succinct, the code for the action is fairly long. Here's the full listing of the resulting `change-issue-status.ts` file:
...
This guide walked you through creating a fully functional third-party integration for OpenOps by building a JetBrains YouTrack block with two actions. You have learned more techniques involved in developing OpenOps blocks, including:
...
* Adding configuration properties to actions, including dynamic, interdependent properties. To learn more about property types that OpenOps supports by virtue of being based on Activepieces, see [Props](https://www.activepieces.com/docs/developers/piece-reference/properties) in Activepieces documentation.
* Using values returned by properties in other properties as well as in the action's execution logic.

To learn more about the internals of OpenOps blocks, consider exploring the [code of existing blocks](https://github.com/openops-cloud/openops/tree/main/packages/blocks) or past pull requests that added new blocks, such as [Ternary](https://github.com/openops-cloud/openops/pull/326), [ServiceNow](https://github.com/openops-cloud/openops/pull/1078), and [nOps.io](https://github.com/openops-cloud/openops/pull/1403).
