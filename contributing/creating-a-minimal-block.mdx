...
Let's see what it takes to create a new OpenOps block, create an action in it, and see it in the OpenOps UI.
...
2. `Package name`: optionally, you can enter a name for the npm package associated with your block. If left blank, the default name will be used, which is: `@openops/block-<block-name>`.
...
* Source and tests:
    * `src/index.ts`: the main entry point of your block. This is where you'll define its metadata (name, display name, description) and register actions.
...
* Project metadata:
    ...
    * `project.json`: a configuration file for the Nx build system that OpenOps uses.
    * `README.md`: a starter readme file where you should describe what the block does and how to use it.
* Tooling and quality:
    * `.eslintrc.json`: ESLint rules for enforcing code style and catching errors.
    * `jest.config.ts`: configuration for Jest, the test runner that OpenOps uses.
* TypeScript setup:
    ...
    * `tsconfig.base.json` (in the repo root): this file serves as a central list of all TypeScript settings, and the CLI updates it to include your new block.
...
Now let's create the first action that will make a simple GET request to [httpbin.org](https://httpbin.org/).
...
1. `Enter the block folder name`: enter be the folder of the block you've just created: `my-unique-block`.
2. `Enter the action display name`: enter the name of the action that users see in the OpenOps UI. This is also used as a base for the name of the file for the action. For this exercise, enter `My first action`.
3. `Enter the action description`: this should be a brief but informative text in the UI, elaborating about the action's function and purpose. For this exercise, it's OK to enter `What this action does`.
...
* The `props` property is an object that defines the properties that the action needs the user to configure. In this case, the action doesn't require any properties.
* The `run()` function is called when the action is executed. It takes a single argument, `context`, which contains the values of the action's properties. In this case, the action doesn't have any properties, and the `context` parameter remains unused.
...
Congratulations! You've just created your first OpenOps block and action. The block doesn't do a lot, and if you're wondering how to develop a block that does something useful, such as implements a working integration with a third-party API, check out the [Contributing an Integration](/contributing/contributing-an-integration/) guide.
