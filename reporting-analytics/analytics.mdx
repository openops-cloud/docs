...
2. Select `openops_tables_connection` for **Database** and `public` for **Schema**. In the **Table** dropdown, select a Postgres DB view corresponding to the OpenOps table you want to source data from. For example, here's what a view may be named for the *Opportunities* table:
...
6. If you make further changes to the chart, click **Update chart** to refresh the preview accordingly.
...
8. Click **Save**. When you go back to the **Analytics** view back in OpenOps, you can see that the chart is now displayed there:
