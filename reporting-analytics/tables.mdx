...
OpenOps tables are fully integrated with OpenOps workflow capabilities: there are [actions](/workflow-management/actions/) that let you create and update records in tables, as well as read data from them. When workflows access tables, OpenOps authenticates using database tokens managed by OpenOps. When workflows access tables, OpenOps authenticates using database tokens managed by OpenOps.
...
1. Go to the **Tables** tab. In the tree view on the left, click **+ Create table** under the list of currently available tables in the **OpenOps Table** dataset:
    ![Creating a table](/images/tables-tree-create.png)
...
![Table field types](/images/tables-add-fields.png)
...
![Table options](/images/tables-table-options.png)
...
![Row identifier field](/images/tables-row-identifier-edit-field.png)
2. In the **Edit field** popup, change the field type to **UUID**:
![Changing the row identifier field type](/images/tables-row-identifier-uuid.png)
...
![Table actions](/images/tables-actions.png)
...
![Specifying the ID from a previous step](/images/tables-actions-id-from-previous-action.png)
