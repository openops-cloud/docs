...
<Note>If you need workflow outputs to land in your organization's existing data warehouse (Snowflake, AWS Athena, BigQuery), you can configure workflows to [write directly to external databases](/reporting-analytics/connecting-to-external-tools#writing-workflow-outputs-to-external-databases) instead of using OpenOps tables.</Note>
...
For guidance on when to use OpenOps tables versus external databases for workflow outputs, see [Writing workflow outputs to external databases](/reporting-analytics/connecting-to-external-tools#writing-workflow-outputs-to-external-databases).
