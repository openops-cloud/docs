...
## Workspaces and access tokens

- Each project owns its Tables workspace (moved from organization to project). This provides isolation per project.
- A Tables database token is generated automatically when a project is created. Built‑in actions use this token; no manual setup is required for default workflows.
